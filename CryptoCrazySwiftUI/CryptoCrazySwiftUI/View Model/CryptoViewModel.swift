...
class CryptoListViewModel : ObservableObject{
    ...
    func downloadCryptosContinuation(url: URL) async {
        do{
            let cryptos = try await webService.downloadCurrinciesContinuation(url: url)
 //MainActor Kullandık         //  DispatchQueue.main.async {
        ...
        webService.downloadCurrencies(url: url) { result in
            switch result{
                ...
                if let cryptos = cryptos{
                    DispatchQueue.main.async {
                        self.cryptoList = cryptos.map(CryptoVewModel.init)
                    }
                }
            }
        }
